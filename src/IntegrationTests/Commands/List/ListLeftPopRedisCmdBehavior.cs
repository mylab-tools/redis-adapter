...
namespace IntegrationTests.Commands.List
{
    ...
    public class ListLeftPopRedisCmdBehavior : IClassFixture<RedisConnectionFixture>
    {
        private readonly RedisConnectionFixture _fixture;

        public ListLeftPopRedisCmdBehavior(RedisConnectionFixture fixture, ITestOutputHelper output)
        {
        ...
        public async Task ShouldPopItemFromEnd()
        {
            ...
            var rPopCmd = new ListLeftPopRedisCmd(key);
            ...
            //Assert
            Assert.Equal("foo", popedItem);

            Assert.Single(listItems);
            Assert.Equal("bar", listItems[0]);
        }
    }
}
